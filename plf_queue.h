...
public:
	...
	explicit queue(const element_allocator_type &alloc):
	...
	queue(const size_type min, const size_type max, const element_allocator_type &alloc):
		element_allocator_type(alloc),
		current_group(NULL),
		first_group(NULL),
		top_element(NULL),
...
public:
	...
	inline PLF_FORCE_INLINE reference front() const // Exception may occur if queue is empty in release mode
	{
		assert(total_size != 0);
	...
	inline PLF_FORCE_INLINE reference back() const // Exception may occur if queue is empty in release mode
	{
		assert(total_size != 0);
		return *top_element;
	}
	...
	void pop() // Exception may occur if queue is empty
	{
		...
		#endif
		{
			PLF_DESTROY(element_allocator_type, *this, start_element);
		}
	...
	inline queue & operator = (const queue &source)
	{
		assert(&source != this);
...
public:
	...
	inline bool operator != (const queue &rh) const PLF_NOEXCEPT
	{
		return !(*this == rh);
	}
	...
	#ifdef PLF_CPP20_SUPPORT
		friend auto operator <=> (const queue &lh, const queue &rh)
		{
	...
	void swap(queue &source) PLF_NOEXCEPT_SWAP(element_allocator_type)
	{
		#ifdef PLF_TYPE_TRAITS_SUPPORT
